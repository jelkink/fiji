...
for (i in 1:NROW(output)) {
  ...
  gsmax <- cbind(1, input.co2$CO2) %*% bhat
  year <- input.co2$Year
  ...
  if (output$replication[i] == 1) {
    ...
    export(annAverages, file = paste0("simulation output - ", output$species[i], " - Rubisco N ", as.character(output$rubisco[i]), " - replication 1.xlsx"))
    save(list = ls(all.names = TRUE), file = paste0("simulation output - ", output$species[i], " - Rubisco N ", as.character(output$rubisco[i]), " - replication 1.Rdata"))
  }

  output$gsmax_co2_slope[i] <- bhat[2]
...
for (i in 1:NROW(out)) {
  out$value[i] <- quantile(output[output$species == out$species[i] & output$rubisco == out$rubisco[i], out$variable[i]], out$quantile[i])
}
export(out, file = "simulation output - vary Rubisco - confidence interval.xlsx")

export(output, file = "simulation output - vary Rubisco.xlsx")

if (Sys.getenv("RSTUDIO") != "1") save.image(file = "simulation output - vary Rubisco.Rdata")
